---
title: "Car Models Project"
author: "di Stefano D'Antoni"
output:
...
Attraverso questo progretto ho voluto studiare le carateristiche principali dei motori delle automobili, sia nel corso del tempo che complessivamente.

Le variabili considerate sono:

- potenza (cv);
- cilindrata (cc);
- rapporto cavalli per litro (HP/L);
- consumo carburante;
- segmento auto;
...
## Dataset utilizzato
I dati provengono dal sito Kaggle, il dataset originario è decisamente ampio e data la presenza di notevoli informazioni non necessarie è stato ridotto e modellato.

### **Questioni studiate:**

- Analisi generale per cilindrata e cavalli;
- Analisi singola cilindrata e cavalli nel tempo;
- Cambiamento rapporto cavlli per litro nel tempo;
- Miglior segmento per prestazioni relative;
- Miglior segmento per consumi;
- Analisi complessiva segmenti;



## Prima analisi ai dati

Dipendenza tra cilindrata e potenza:

```{r echo=FALSE, message=FALSE}
grafico_dispersione
```

Con il passare del tempo le auto tendono ad avere più cavalli e meno cilindrata.

Data la tendenza vista segue un'analisi singola per i due valori.

## Analisi temporale singola 

Il primo valore tende a calare, mentre nel secondo caso è presente una buona tendenza crescente.

```{r echo=FALSE, message=FALSE}
grid.arrange(cilindrataNelTempo, potenzaNelTempo, ncol = 2)
```


## Medie annuali Cavalli al litro con Normative Euro

L'introduzione di norme sulle emissioni sempre più stringenti ha portato ad un'elevata quantità di filtri sulla linea di scarico, che riducono però la potenza erogata.

Dunque le case produttrici, per mantenere i valori dei loro modelli, devono aumentare leggermente le prestazioni del motore. L'opzione più semplice è di aumentare la cilindrata.

Probabilmente anche per questo motivo il grafico dell'andamento medio di questo valore nel tempo, mostrava una riduzione relativamente contenuta.


```{r echo=FALSE, message=FALSE}
plotline_averages_smallTimeGap
```


## Miglior segmento per prestazioni

```{r echo=FALSE, message=FALSE}
segments_plot
```

- **Segmento Migliore:** Exotic (es. LAMBORGHINI Aventador SVJ, FERRARI SF90)
- **Segmento Peggiore:** Pickup (es. FORD Ranger, DODGE Ram)

Dato il prezzo elevato delle auto esotiche (supercar, hypercar), queste hanno motori molto sviluppati che garantiscono prestazioni notevoli.


Il segmento dei pickup è il peggiore in quanto l'aspetto importante per un motore di quel tipo è la coppia e non i cavalli.
...
- **Segmento Migliore:** Small (es. HUNDAI i10, PEUGEOT 207)
- **Segmento Peggiore:** Exotic (es. LAMBORGHINI Aventador SVJ, FERRARI SF90)



## Analisi complessiva
Questo grafico riassume le informazioni più importanti analizzate fino ad ora.
Pone in relazione i diversi segmenti di auto, utilizzando i cavalli per litro sull'asse x, il consumo del carburante sull'asse y.
Inoltre la grandezza dei punti indica un'approssimazione del valore di cilindrata.

```{r echo=FALSE, message=FALSE}
HpL_Consumption_dispersionPlot
```
